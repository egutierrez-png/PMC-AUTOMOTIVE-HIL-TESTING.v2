...
export default function StepChecklist() {
  ...
  useEffect(() => {
    ...
    setSteps((prev) => {
      ...
      results.results.forEach((res: Step, resultIndex: number) => {
        const resultStep = resolveResultStep(res, resultIndex);
        const idx = merged.findIndex((s) => s.step === resultStep);
  ...
  useEffect(() => {
    ...
    setSteps((prev) => {
      ...
      resultArray.forEach((res: any, resultIndex: number) => {
        const resultStep = resolveResultStep(res, resultIndex);
        const idx = merged.findIndex((s) => s.step === resultStep);
        if (idx < 0) return;
