...
import OperatorQualityPanel from "./components/OperatorQualityPanel";
...
export default function App() {
  ...
  if (tab === "run" || tab === "signals" || tab === "operator") return true; // run siempre libre
  ...
  if (user === null) {
    setTab("operator"); // <-- regresar automáticamente a RUN
  }
}, [user]);
...
// Efecto para manejar navegación pendiente tras login
  ...
  return (
    ...
    >
      ...
      {tab === 'operator' && (
        <>
        {/* Aquí va la columna con los modos, selector de modelo y controles principales */ }
        <div className="
          ...
          <div className="col-span-1 h-full">
            <ModeSelectorPanel showDescription={false} className="h-full" /> 
          </div>

          {/* Centro: controles y parámetros de ejecución */}
          <div className="col-span-1 h-full">
            <RecipeSelector className="h-full" />
          </div>
          ...
          <div className="col-span-1 md:col-span-2 lg:col-span-1 h-full">
            <ControlsPanel
              className="h-full"
              layout="horizontal"
              visibleButtons={{ queryStatus: false }}
            />
          </div>
        </div>

        {/* Aquí se encuentra el panel de status y los resultados principales” */}
        <div className="
          mt-4 md:mt-6
          grid gap-4 md:gap-6
          grid-cols-1 md:grid-cols-2 lg:grid-cols-6 xl:grid-cols-6">

          {/* Columna izquierda tipo “árbol de pruebas/estados” */}
          <div className="col-span-1 md:col-span-2 lg:col-span-2 xl:col-span-2 flex flex-col gap-3">
            <StatusPanel layout="horizontal" />
          </div>

          {/* Centro: controles y parámetros de ejecución */}
          <div className="col-span-1 md:col-span-2  lg:col-span-4 xl:col-span-4 flex flex-col gap-3">
            <StepChecklist />
          </div>
        </div>
